...
// Cache for Linking module to avoid repeated imports
let linkingModule: any = null;
let linkingPromise: Promise<any> | null = null;
...
export class UrlHandlerService {
  /**
   * Get Linking module with caching to prevent memory leaks
   */
  private static async getLinkingModule(): Promise<any> {
    if (linkingModule) {
    ...
    linkingPromise = import('react-native').then(module => {
      linkingModule = module.Linking;
      linkingPromise = null;
      return linkingModule;
    });
